...
#include "../Il2CppOutputProject/IL2CPP/libil2cpp/il2cpp-api.h"
...
static NSString* const kOverrideUnityConfigDidChangeNotification = @"OverrideUnityConfigDidChange";
static NSString* const kOverrideServerURLDefaultsKey = @"redflag.server_url";
static NSString* const kOverrideTeamIDDefaultsKey = @"redflag.team_id";
static NSString* const kOverrideUsernameDefaultsKey = @"redflag.username";
static id _overrideUnityConfigObserver = nil;

static NSString* OverrideTrimmedDefaultsString(NSString* key)
{
...
static void OverrideLogManagedException(NSString* context, Il2CppException* exception)
{
    ...
    NSLog(@"[OverrideUnity] %@ failed: %s", context, message);
}

static void OverrideSetManagedStringField(Il2CppObject* instance, Il2CppClass* klass, const char* fieldName, NSString* value)
{
    FieldInfo* field = il2cpp_class_get_field_from_name(klass, fieldName);
    if (field == nullptr)
        return;

    Il2CppObject* managedValue = nullptr;
    if (value != nil)
        managedValue = reinterpret_cast<Il2CppObject*>(il2cpp_string_new(value.UTF8String));

    il2cpp_field_set_value_object(instance, field, managedValue);
}

static bool OverrideInvokeBridgeMethod(Il2CppObject* instance, Il2CppClass* klass, const char* methodName)
{
    const MethodInfo* method = il2cpp_class_get_method_from_name(klass, methodName, 0);
    if (method == nullptr)
        return false;

    Il2CppException* exception = nullptr;
    il2cpp_runtime_invoke(method, instance, nullptr, &exception);
    if (exception != nullptr)
    {
        OverrideLogManagedException([NSString stringWithUTF8String: methodName], exception);
        return false;
    }
...
static bool OverrideApplyBridgeManagerConfig(void)
{
    ...
    Il2CppClass* bridgeClass = il2cpp_class_from_name(image, "", "BridgeManager");
    if (bridgeClass == nullptr)
        return false;

    FieldInfo* instanceField = il2cpp_class_get_field_from_name(bridgeClass, "Instance");
    if (instanceField == nullptr)
        return false;

    Il2CppObject* bridgeInstance = nullptr;
    il2cpp_field_static_get_value(instanceField, &bridgeInstance);
    if (bridgeInstance == nullptr)
        return false;

    OverrideSetManagedStringField(bridgeInstance, bridgeClass, "serverUrl", OverrideTrimmedDefaultsString(kOverrideServerURLDefaultsKey));
    OverrideSetManagedStringField(bridgeInstance, bridgeClass, "teamId", OverrideTrimmedDefaultsString(kOverrideTeamIDDefaultsKey));
    OverrideSetManagedStringField(bridgeInstance, bridgeClass, "username", OverrideTrimmedDefaultsString(kOverrideUsernameDefaultsKey));

    FieldInfo* isConnectedField = il2cpp_class_get_field_from_name(bridgeClass, "isConnected");
    if (isConnectedField != nullptr)
    {
        bool isConnected = false;
        il2cpp_field_set_value(bridgeInstance, isConnectedField, &isConnected);
    }

    OverrideInvokeBridgeMethod(bridgeInstance, bridgeClass, "OnDestroy");

    FieldInfo* socketField = il2cpp_class_get_field_from_name(bridgeClass, "socket");
    if (socketField != nullptr)
        il2cpp_field_set_value_object(bridgeInstance, socketField, nullptr);

    return OverrideInvokeBridgeMethod(bridgeInstance, bridgeClass, "InitSocket");
}

static void OverrideScheduleBridgeManagerSync(NSInteger attemptsRemaining)
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if (OverrideApplyBridgeManagerConfig() || attemptsRemaining <= 0)
            return;

        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.35 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            OverrideScheduleBridgeManagerSync(attemptsRemaining - 1);
        });
    });
}

static void OverrideRegisterUnityConfigObserverIfNeeded(void)
{
    if (_overrideUnityConfigObserver != nil)
        return;

    _overrideUnityConfigObserver = [NSNotificationCenter.defaultCenter addObserverForName: kOverrideUnityConfigDidChangeNotification
                                                                                    object: nil
                                                                                     queue: NSOperationQueue.mainQueue
                                                                                usingBlock: ^(__unused NSNotification* note) {
        OverrideScheduleBridgeManagerSync(8);
    }];
}
...
- (void)startUnity
{
    ...
    // Keep poster audio audible even when Unity boots without its own audio manager.
    // Ambient would respect the mute switch and can silence the hidden web/AVPlayer path.
    if (!UnityIsAudioManagerAvailableAndEnabled())
    {
        [audioSession setCategory: AVAudioSessionCategoryPlayback error: nil];
        [audioSession setActive: YES error: nil];
    }
    OverrideRegisterUnityConfigObserverIfNeeded();
    OverrideScheduleBridgeManagerSync(8);
